...
def _v4_attention_alias_apply(mod):
    ...
    def _bf16_insert(q, kv, swa_kv_cache_3d, slot_mapping, positions, cos_sin, eps, block_size):
        # BF16 full-cache path fallback:
        #   Q side: per-head RMSNorm (no weight) + GPT-J RoPE (in-place)
        #   KV side: GPT-J RoPE + bf16 paged cache insert (no quant)
        import math

        # q: [N, H, 512], kv: [N, 512]
        num_tokens = q.shape[0]
        n_heads = q.shape[1] if q.ndim == 3 else 1
        head_dim = 512
        rope_dim = 64  # last 64 dims get RoPE

        # Q: per-head RMSNorm (no weight)
        if q.ndim == 3:
            q_float = q.float()
        ...
        # GPT-J RoPE on last rope_dim dims of Q and KV
        cos_sin_selected = cos_sin[positions]  # [N, 64]
        cos_val = cos_sin_selected[:, :32]     # [N, 32]
        sin_val = cos_sin_selected[:, 32:]     # [N, 32]

        def apply_gptj_rope(x_rope):
            # x_rope: [..., 64], interleaved pairs
            x1 = x_rope[..., ::2]   # even indices
            x2 = x_rope[..., 1::2]  # odd indices
            cos_b = cos_val.view(*([1]*(x_rope.ndim-2)), num_tokens, 32) if x_rope.ndim > 2 else cos_val
            sin_b = sin_val.view(*([1]*(x_rope.ndim-2)), num_tokens, 32) if x_rope.ndim > 2 else sin_val
            if x_rope.ndim == 3:
                # q case: [N, H, 64] -> cos/sin need [N, 1, 32]
            ...
            out = torch.stack([out1, out2], dim=-1).flatten(-2).to(x_rope.dtype)
            return out

        # Apply RoPE to Q last 64 dims
        if q.ndim == 3:
            q[..., -rope_dim:] = apply_gptj_rope(q[..., -rope_dim:])
        else:
            q[..., -rope_dim:] = apply_gptj_rope(q[..., -rope_dim:])

        # Apply RoPE to KV last 64 dims
        kv_roped = kv.clone()
        kv_roped[..., -rope_dim:] = apply_gptj_rope(kv[..., -rope_dim:])

        # Write into paged cache
        # swa_kv_cache_3d: [num_blocks, block_size, 512]
        for i in range(num_tokens):
            ...
            swa_kv_cache_3d[block_idx, offset, :] = kv_roped[i]
...
def _hash_router_apply(mod):
    fn = mod._topk_softplus_sqrt_torch
    fn._kunlun_hash_fallback = True
    mod.vllm_topk_softplus_sqrt = fn
...
def _compressor_save_apply(mod):
    ...
    logging.getLogger("vllm_kunlun").info(
        "[KunlunPlugin] patched V4 save_partial_states (PyTorch fallback)"
    )
