...
def _torch_pre(residual, fn, hc_scale, hc_base, rms_eps, hc_pre_eps,
    ...
    comb = comb_raw.reshape(-1, hc_mult, hc_mult)
    for _ in range(sinkhorn_repeat):
        comb = comb.softmax(dim=-1)
        comb = comb / comb.sum(dim=-2, keepdim=True).clamp_min(hc_sinkhorn_eps)
    pre = pre / pre.sum(dim=-1, keepdim=True).clamp_min(hc_sinkhorn_eps)
    post = post * hc_post_mult_value
    layer_input = torch.bmm(pre.unsqueeze(1), residual.reshape(-1, hc_mult, hidden_size)).squeeze(1)
    layer_input = _rms_norm(layer_input, norm_weight, norm_eps)
    return (
